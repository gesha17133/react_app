import classes from './Navbar.module.css';
import {useState} from 'react'
import {FaArrowLeft, FaUserCog} from 'react-icons/fa';
import {FaMapMarkerAlt} from 'react-icons/fa';
import {FaRegChartBar} from 'react-icons/fa';
import {FaUserFriends} from 'react-icons/fa';
import {FaClipboardList} from 'react-icons/fa';
import {FaTasks} from 'react-icons/fa';
import {NavLink} from 'react-router-dom';
import { Switcher } from './Switcher';


const Navbar = () => {
    const [state, setState] = useState({})
    const switherAsideToggle = () => {
        setState(!state)
    }
    return(
        <aside className={`new ${classes.navbarWrapper} ${state ? classes.asSwitched : ""} `}>
            <LopNavbar />
            <Switcher switherAsideToggle={switherAsideToggle}/>
            <Menu />
            <div className={classes.navbarBackground}></div>
            <div className={classes.NavbarDecor_1}></div>
            <div className={classes.NavbarDecor_2}></div>
            <div className={classes.NavbarDecor_3}></div>
            <div></div>
            <div></div>
        </aside>
    );
}

const LopNavbar = () => {
    return(
        <div className={classes.navbarTop} >
            <img className={classes.logoImg} src="data:image/png;base64," alt=""/>
            <div className={classes.logoText}>Project-X</div>
        </div>
    );
}

const MenuItem = (props) => {
    return(
        <li className = {classes.menuItem} ><NavLink activeStyle={{ background: 'rgba(17, 236, 229,0.8)' }} to={props.link} className={classes.Navigator}>{props.icon} <span > {props.message}</span></NavLink></li>    
    )
}

const Menu = () => {

    let MenuItems = [
        { link: '/dashboard', icon: <FaClipboardList/>, message: 'Dashboard' },
        { link: '/profile',   icon: <FaUserCog/>, message: 'Profile' },
        { link: '/chat',      icon: <FaUserFriends/>, message: 'Chats' },
        { link: '/tasker',    icon: <FaTasks/>, message: 'Tasker' },
        { link: '/stats',     icon: <FaRegChartBar/>, message: 'statistics' },
        { link: '/maps',      icon: <FaMapMarkerAlt/>, message: 'Maps' },
    ];

    return (
        <ul className = { classes.navbarMenu } >
            { MenuItems.map ( ( item ) => {
                return <MenuItem  className={classes.SvgIconMenu} link={item.link} icon={item.icon} message ={item.message} />
            } ) }
        </ul>
    );
}

export default Navbar;

/*https://openbase.com/js/react-chat-elements*/